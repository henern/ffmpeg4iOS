...
set -e
if [ ! -e openssl-${VERSION}.tar ]; then
	echo "Downloading openssl-${VERSION}.tar"
    curl -O https://www.openssl.org/source/openssl-${VERSION}.tar
else
	echo "Using openssl-${VERSION}.tar"
fi
...
tar zxf openssl-${VERSION}.tar -C "${CURRENTPATH}/src"
